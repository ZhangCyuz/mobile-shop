    ...
    shop.api.fetchCart(function(response){
      ...
      if (response.data.length > 0) {
        ...
        for(var i = 0; i < obj.length; i++){
          ...
          $(".cart table").append('<tbody class="good">'
            +'<tr><td class="good-first"><input type="checkbox" class="chkbox"><input type="hidden" class="goods_id" value="${obj.goods_id}"></td>'
            +'<td class="good-second"><img src="' + obj[i].goods_thumb + '"></td>'
            +'<td class="span">'+obj[i].goods_name +'</td>'
            +'<td class="good-price">' + obj[i].goods_price +'</td>'
            +'<td class="good-for"><button onclick="minusGoods(this)">-</button>'
            + '<input class="goods_number" type="text" value="'+ obj[i].goods_number+'">'
            + '<button onclick="plusGoods(this)">+</button></td>'
            +'<td class="subtotal">'+ obj.subtotal +'</td></tr>'
            +'<div class="del"><span  onclick="deleteGoods(this)">'+'删除' +'</span></div>'
            + '</tbody>');
        }
        showSum();
      }
...
function showSum() {
  ...
  for (var i = 0; i < trs.length; i++) {
    ...
    if ($(tr).children("td:first").children("input").is(':checked')) {
        ...
        sum += parseInt($(tr).children('td:eq(5)').text());
    }
  }
  $('.money-num').text(sum);
}
